 ...
 * URL.
 *
 * @note While fetching the content, it will present the ModSecurity instance to
 *       the remote server, trough: ModSecurity Unique ID, ModSecurity status
 *       line and also, if given, key that can be used to authentication.
 *       Such data is presented in the following HTTP headers:
 *         - ModSec-status
 *         - ModSec-unique-id
 *         - ModSec-key
 *
    ...
    struct msc_curl_memory_buffer_t *chunk, char **error_msg)
{
    ...
    if (curl)
    {
        ...
        if (res != CURLE_OK)
        {
            *error_msg = apr_psprintf(mp, "Failed to fetch \"%s\" error: %s ", remote_rules_server->uri, curl_easy_strerror(res));
            return -1;
        }
        ...
        char **error_msg)
{
    ...
    const int do_pad = 1;
    const apr_crypto_block_key_type_e type = APR_KEY_AES_256;
    const apr_crypto_block_key_mode_e mode = APR_MODE_CBC;
    apr_status_t rv;
 ...
 * @param chunk_plain msc_curl_memory_buffer_t to be filled with the plain text.
        ...
        char **error_msg)
{
...
#else
    rv = apr_crypto_get_driver(&driver, APU_CRYPTO_RECOMMENDED_DRIVER, NULL, &err, pool);
#endif
    ...
    if (rv == APR_ENOKEY)
    {
        *error_msg = "Internal error - apr_crypto_block_decrypt_init: Missing key";
        return -1;
    }
    else if (rv == APR_ENOIV)
    {
        *error_msg = "Internal error - apr_crypto_block_decrypt_init: Missing IV";
        return -1;
    }
    else if (rv == APR_EKEYTYPE)
    {
        *error_msg = "Internal error - apr_crypto_block_decrypt_init: Wrong key type";
        return -1;
    }
    else if (rv == APR_EKEYLENGTH)
    {
        *error_msg = "Internal error - apr_crypto_block_decrypt_init: Wrong key length";
        return -1;
    }
    else if (rv != APR_SUCCESS)
    {
        *error_msg = "Internal error - apr_crypto_block_decrypt_init: Unknown error";
        return -1;
    }

    //FIXME: size should not be hardcoded like that.
    rv = apr_crypto_block_decrypt(&plain_text, &plain_text_len,
        (unsigned char *) ciphered_text, (apr_size_t) chunk->size - (16 + 16), block);
    if (rv != APR_SUCCESS)
    {
        *error_msg = "Internal error - apr_crypto_block_decrypt: Failed to decrypt";
        return -1;
    }

    /* finalise the decryption */
    rv = apr_crypto_block_decrypt_finish(plain_text + plain_text_len, &len, block);
    if (rv != APR_SUCCESS)
    {
        *error_msg = "Internal error - apr_crypto_block_decrypt_finish: Failed to decrypt";
        return -1;
    }

    chunk_plain->size = plain_text_len + len;
    chunk_plain->memory = plain_text;
 ...
 * @param remote_rules_server pointer to the filled msc_remote_rules_server structure.
 * @param error_msg pointer to char pointer that will be filled if something went wrong.
 *
 *
 * @retval n>=0 everything went fine.
 * @retval n<-1 Something wrong happened, further details on error_msg.
        ...
        char **error_msg)
{
    ...
    msc_remote_grab_content(mp, remote_rules_server->uri, remote_rules_server->key,
            &chunk_encrypted, error_msg);
    if (*error_msg != NULL)
    {
        return -1;
    }

    msc_remote_decrypt(mp, remote_rules_server->key, &chunk_encrypted, &chunk, error_msg);
    if (*error_msg != NULL)
    {
        return -1;
    }
    ...
    while (len < chunk.size)
    {
        ...
        {
            int tmp = len;
            const char *rule = chunk.memory + start;
            char *cmd_name;
            char *word;
            ...
            if (cmd == NULL)
            {
                *error_msg = apr_pstrcat(mp, "Unknown command in config: ", cmd_name, NULL);
                return -1;
            }
            ...
            {
#endif
                *error_msg = (char *) msc_remote_invoke_cmd(cmd, parms, remote_rules_server->context, rule);
                if (*error_msg != NULL)
                {
                    return -1;
                }
            ...
            {
                error_msg = "Command failed to execute (check file/folder permissions, syntax, etc.).";
                return -1;
            }
#endif
...
/**
 * @brief Ping back the rules server to ask for new rules
 *
 * From time to time ping back the rules server, asking for more rules.
 * THIS IS NOT WORKING.
 *
 * @warning this function is not working, it is a prototype. 
 *
 *
 */
 /*
  * Disabled to avoid compilation issues, not ready for production yet.
  */
#if 0
void msc_remote_rules_after_logging(modsec_rec *msr) {
    ...
    if (!msr || !msr->dcfg1 || !msr->dcfg1->remote_rules)
        return;


    time(&raw_time);

    double diff = difftime(raw_time, msr->dcfg1->remote_rules->timeout);

    if (diff < 15) {
        return;
    }

    time(&(msr->dcfg1->remote_rules->timeout));
    */
    return;
}
#endif
